...
```bash
echo 'export MONIKER="Your Moniker Brandname"' >> ~/.bash_profile
...
Wait a bit until you get the logs. To get out of them press `Ctrl+C`.

### Install Spanshot, addrbook, update peers:

```bash
curl -Ls https://snapshots.raylord.best/snapshots/testnet/zero-gravity/addrbook.json > $HOME/.0gchain/config/addrbook.json
```

```bash
PEERS=$(curl -s --max-time 3 --retry 2 --retry-connrefused "https://snapshots.raylord.best/snapshots/testnet/zero-gravity/peers.txt")
if [ -z "$PEERS" ]; then
...
curl -L http://snapshots.raylord.best/snapshots/testnet/zero-gravity/zgtendermint_16600-1_latest.tar.lz4 | tar -Ilz4 -xf - -C $HOME/.0gchain
...
### For a validator to join an active set, you need at least 10000000000000000000aevmos
